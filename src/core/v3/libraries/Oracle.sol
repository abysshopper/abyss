...
library Oracle {
    ...
    function transform(
        ...
        // Canonical oracle accumulators and uint32 timestamps intentionally wrap.
        unchecked {
    ...
    function grow(Observation[65_535] storage self, uint16 current, uint16 next)
        internal
        returns (uint16)
    {
        if (current == 0) revert InvalidObservationState();
    ...
    function getSurroundingObservations(
        ...
        if (!lte(time, beforeOrAt.blockTimestamp, target)) revert ObservationTooOld();
    ...
    function observeSingle(
        ...
        uint32 target;
        unchecked {
            // uint32 timestamps use modular arithmetic across the 2**32 boundary.
        ...
        if (target == beforeOrAt.blockTimestamp) {
            ...
            // we're in the middle
            unchecked {
                // Observation timestamps and accumulators are canonical modular counters.
